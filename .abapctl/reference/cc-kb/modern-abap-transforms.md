<!-- Source: https://github.com/SAP/styleguides/blob/main/clean-abap/sub-sections/ModernABAPLanguageElements.md -->
<!-- License: Apache 2.0 (https://github.com/SAP/styleguides/blob/main/LICENSE) -->
<!-- Downloaded: 2026-03-27 -->
<!-- Local addition (not upstream): the "Describe data types" subsection (DESCRIBE FIELD -> RTTI), migrated from the removed clean-core-remediation-patterns.md -->
...
Inspect a data object's type at runtime with the
[RTTI](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/abenrtti.htm)
class hierarchy (`cl_abap_typedescr` and its subclasses).

```ABAP
DATA(type_descr) = cl_abap_typedescr=>describe_by_data( field ).
DATA(length)     = CAST cl_abap_elemdescr( type_descr )->output_length.
```

Old style — the obsolete `DESCRIBE FIELD` statement:

```ABAP
DATA length TYPE i.
DESCRIBE FIELD field LENGTH length IN CHARACTER MODE.
```
